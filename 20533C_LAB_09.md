...
In addition to these tasks, A. Datum wants you to evaluate some of the advanced features Azure AD Premium offers. Additionally, it wants you join a Windows 10â€“based computer to an Azure AD tenant to test the Azure AD functionality and prepare for implementing this configuration on all the Windows 10â€“based computers in the Research department.
...
- Configure SSO from a Windows 10â€“based computer that is joined to Azure AD.
...
3. Add a directory by using the following settings:
  ...
  -  **NAME**: Adatum
  -  **DOMAIN NAME**:  _Use your initials + the_ _directory name_ _ + random numbers (e.g. abcadatum123456)_
  -  **COUNTRY** **O** **R REGION**: United States
...
1. Start  **Windows PowerShell** **Â **ISE **Â **as an administrator.
2. Open  **D:\Labfiles\Lab0** **9** **\Starter\ExampleCommands.ps1**.
...
5. In the PowerShell ISE, in the script pane, locate the following code:
  ...
  ```
  New-MsolUser -UserPrincipalName mledford@<#Copy your Azure Directory name here#>.onmicrosoft.com  -DisplayName â€œMario Ledfordâ€ -FirstName â€œMarioâ€ -LastName â€œLedfordâ€ -Password â€˜Pa$$w0rd123â€™ -ForceChangePassword $false -UsageLocation â€œUSâ€
  ```
...
8. Create a new group by running the following command:
  ...
  ```
  New-MsolGroup -DisplayName â€œAzure teamâ€ -Description â€œAdatum Azure team usersâ€
  ```
...
3. Go to  **https://account.activedirectory.windowsazure.com/applications**, and sign in by using the following credentials (where  **_XXXadatumXXX_**_Â _is your unique Adatum domain name):
...
3. On the  **a** **dditional** **s** **ecurity** **v** **erification** page, note the contact method options.
4. Optional step: If you have access to a mobile phone in the classroom, and have a signal or data connection, you can complete the additional security verification steps on the  **a** **dditional** **s** **ecurity** **v** **erification** page.

>  **Result**: After completing this exercise, you should have configured Multi-Factor Authentication for administrators.


## Exercise 4: Configuring SSO from a Windows 10â€“based computer that is joined to Azure AD
  
### Scenario
  A. Datum has an increasing demand to provide its remote and mobile users, who are using Windows 10â€“based devices, with secure access to the cloud resources. The company wants to join Windows 10 devices to Azure AD and simplify access to cloud resources by enabling SSO. Before they can implement this, you want to test this functionality by joining a WindowsÂ 10â€“based computer to Azure AD. 
The main tasks for this exercise are as follows:

1. Join a Windows 10-based computer to Azure AD
2. Authenticate to Azure from a Windows 10 Azureâ€“joined computer
...
3. On MIA-CL1, click  **S** **ettings**, click  **A** **ccounts**, **Â **and then join MIA-CL1 into Azure AD by using the following credentials:

  - Username:  **kgruber@XXXadatumXXX.onmicrosoft.com**
  - Password:  **Pa$$w0rd123**

1. Verify that  **M** **IA** **-C** **L** **1** is shown in the **Device** tab of the **Karen Gruber** user account.


#### Task 2: Authenticate to Azure from a Windows 10 Azureâ€“joined computer
  
1. Sign in to MIA-CL1 with Karen Gruberâ€™s credentials:
...
**Question** What is the major benefit of joining Windows 10â€“based devices to Azure AD?
