...
### Scenario
  Managers at A. Datum are planning to migrate some of the companyâ€™s application databases to the cloud.
...
### Scenario
  The operations team at A. Datum currently uses a Microsoft SQL Server database to store inventory of companyâ€™s servers. You want to investigate the option of using Azure SQL Database to host this database. The operations team is interested in monitoring the performance of this database in Azure. 
...
3. On the  **sales** **Â **database blade, identify the value of the  **ADO.NET** database connection string and copy it to the Clipboard.
...
4. Wait for the restore operation to complete by monitoring  **Notifications** area in the portal or on the **Audit Logs** **Â **blade (this can take several minutes).
...
**Question** If the SalesApp web application was deployed to a server with a fixed public IP address, how could you enable it to access the sales Azure SQL Database without allowing it to access any other Azure SQL Database on the same server running SQL?
