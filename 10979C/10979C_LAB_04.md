...
2.   From the Azure portal, create a new  **WordPress** website based on the Azure Marketplace offering from **WordPress**, and then specify the following configuration:
  ...
  -   Resource Group:  **AdatumWebRG**
    ...
    1.   App Service plan:  **AdatumLabSP**.

    2.   Location: The Azure region closest to your classroom location

    3.   Pricing tier:  **D1 Shared**

  -   Database

    1.   Database Name:  **adatumwpdb**

    2.   Database Type:  **Shared**

    3.   Location: The same location you chose for the App Service plan

    4.   Pricing Tier:  **Mercury**
...
1.   From the WordPress dashboard, create a new post with the following settings:
  ...
  Click  **Publish** to publish the new post.


1.   View the new post.

2.   Close the Internet Explorer tab displaying the WordPress interface, and then return to the Azure portal tab.
...
1.   Wait until the cloud service is created, which should take less than a minute. After the deployment is completed, the Azure portal will automatically open the cloud service blade.
...
1.   Wait until the storage account is set up.

2.   From the Azure portal, deploy a package and configuration files to the production slot of a newly created cloud service by using the following settings: 
